...
public class BoardTest {
	...
	public void testDefensiveCopyFromGetSquaresIsAccurate() throws Exception {
		...
		HardcodedMineInitializationStrategy mineDetails = new HardcodedMineInitializationStrategy();
		List<Point> mines = mineDetails.mines(new Point(0,0));
		for(int row=0; row<squaresFromGetSquares.length; row++) {
			...
			for(int col=0; col<squareRow.length; col++) {
				...
				assertEquals(mineDetails.getSquareCount(squarePoint), square.getCount());
